...
def inducer_IPTG(t,y):
    production = 0.0 + ((6.817e-08*(y[6]**2))/(7.702e-05**2 + y[6]**2))
    degradation = 0.0058 * y[3]
    return production - degradation
...
f = open(outfile, 'w')
header = ['time', 'AHL', 'LasR_mRNA', 'LasR', 'GFP_mRNA', 'GFP', 'LasR.AHL', 'IPTG_LasR']
f.write(','.join(header) + '\n')
